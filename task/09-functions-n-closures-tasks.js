...
function getPolynom() {
  ...
  return (x) => {
    return args.reduce((prev, cur, idx) => prev + cur * Math.pow(x, idx), 0);
  };
}
...
function logger(func, logFunc) {
  return function () {
    ...
    const callString = `${func.name}(${JSON.stringify(args).slice(1, -1)})`;
...
function partialUsingArguments(fn, ...initialArgs) {
  return (...additionalArgs) => fn(...initialArgs, ...additionalArgs);
}
